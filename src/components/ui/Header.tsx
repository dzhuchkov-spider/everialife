...
const Header: React.FC<HeaderProps> = ({
  ...
  const Logo: React.FC<{ property1?: 'Mob' | 'Web' }> = ({ property1 = 'Mob' }) => {
    ...
    return (
      <div className={`relative ${isWeb ? 'h-[46px] w-[421px]' : 'h-8 w-[210px]'}`}>
        ...
        <div className={`absolute ${isWeb ? 'inset-[0_49.9%_0_0]' : 'top-1/2 -translate-y-1/2 h-[28px] left-0 overflow-hidden right-[38.87%]'}`}>
          <img 
  ...
  const ButtonCall: React.FC = () => (
    <div className="bg-[#386df7] relative rounded size-8 cursor-pointer" onClick={onCallClick}>
  ...
  const UserInfo: React.FC = () => (
    <div className="flex items-center gap-3">
      <div className="bg-[#437aec] flex items-center justify-center overflow-hidden relative rounded-full w-8 h-8 flex-shrink-0">
        <div className="flex flex-col justify-center h-full font-normal not-italic relative text-sm text-white text-center tracking-wide w-[19px] leading-[1.4]">
          {userName}
        </div>
      </div>
  ...
  const SearchInput: React.FC = () => (
    <div className="flex flex-col h-14 items-start min-w-0 relative flex-1">
      <div className="flex flex-col h-14 items-start justify-center relative flex-shrink-0 w-full">
        <div className="border border-[#b3b3b3] border-solid flex gap-2 h-14 items-center px-2 py-2 relative flex-shrink-0 w-full">
          <div className="bg-[#f2f2f2] flex h-10 items-center justify-center overflow-hidden px-1 relative flex-shrink-0">
            <div className="flex h-6 items-center justify-center px-2 py-0.5 relative flex-shrink-0">
              <span className="font-normal not-italic relative text-sm text-[#4d4d4d] tracking-wide whitespace-nowrap leading-[1.4]">
                по номеру
              </span>
            </div>
            ...
            <div className="flex items-center relative flex-shrink-0 w-full">
              <div className="flex flex-1 items-center min-w-0 overflow-hidden relative">
                <input
                  ...
                  className="font-normal not-italic relative text-base text-[#666] tracking-wide whitespace-nowrap border-none outline-none bg-transparent w-full"
                />
              </div>
            </div>
          </div>
  ...
  const ContactInfo: React.FC = () => (
    <div className="flex flex-col font-normal not-rial h-[37px] items-end leading-0 relative flex-shrink-0 w-[144px] whitespace-nowrap">
      ...
      <div className="flex flex-col justify-center relative flex-shrink-0 text-xs text-[#666] tracking-wide leading-4">
        {supportText}
      </div>
    </div>
  );
